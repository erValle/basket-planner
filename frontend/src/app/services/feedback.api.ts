...
export class FeedbackApiService {
    ...
    canProvideFeedback(versionId: number): Observable<CanProvideFeedbackResponse> {
        return this.api.get<CanProvideFeedbackResponse>(
            `/api/feedbacks/version/${versionId}/can-provide`,
        );
    }
    ...
    listForVersion(versionId: number): Observable<FeedbackSurveyListResponse> {
        return this.api
            .get<ApiFeedbackRow[]>(`/api/feedbacks?trainingPlanVersionId=${versionId}`)
            .pipe(
                map((rows: ApiFeedbackRow[]) => {
