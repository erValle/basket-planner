...
export class ExercisePreviewDialog {
  ...
  calculateAverageDifficulty(): number {
    ...
    const values = [difficulty.tactica, difficulty.tecnica, difficulty.fisica, difficulty.mental].filter(
      (v) => typeof v === 'number'
    );
  ...
  getTagsArray(): string[] {
    ...
    if (typeof this.exercise.tags === 'object') {
      return Object.values(this.exercise.tags).flat().filter((t) => typeof t === 'string');
    }
    return [];
  }
}
