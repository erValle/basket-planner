...
export class Teams {
    ...
    coachOptions: Array<{ label: string; value: number | null }> = [
        { label: 'Sin entrenador', value: null },
    ];

    // Note: initialized empty; set when clubs are loaded.
    clubOptionsWithAll: Array<{ label: string; value: 'all' | number }> = [
    ...
    teams$ = this.refresh$.pipe(
        switchMap(() => {
            ...
            const category =
                this.filters.category !== 'all' ? String(this.filters.category) : undefined;
    ...
    loadClubs(): void {
        this.clubsApi.list().subscribe({
            next: (items) => {
                this.clubs = items ?? [];
                this.clubOptions = (items ?? []).map((c) => ({
                    label: c.name,
                    value: Number(c.id),
                }));
                this.clubOptionsWithAll = [
                    { label: 'Todos', value: 'all' },
                    ...this.clubOptions.map((o) => ({ label: o.label, value: o.value })),
                ];
        ...
        if (this.dialogMode === 'create') {
            ...
            if (!clubId) {
                this.toast.add({
                    severity: 'warn',
                    summary: 'Atención',
                    detail: 'Debes seleccionar un club.',
                });
                return;
            }
            ...
            const payload = {
                ...
                active: this.draft.status === 'active',
            };
