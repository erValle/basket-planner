...
import {
    ...
    ExerciseForm,
    ExerciseFormValue,
} from '../components/exercise-form/exercise-form';
...
const CATEGORY_TO_TYPES: Record<string, string[]> = {
    ...
    'Físico / Recuperación': ['CONDICIONAMIENTO_FISICO', 'MOVILIDAD_RECUPERACION'],
    Otros: ['REBOTE', 'ATAQUE_INDIVIDUAL'],
};
...
export class Exercises implements OnInit {
            ...
            const filtered = items.filter((exercise) => {
                ...
                if (!matchesDuracion) {
                    ...
                    switch (this.filters.duracion) {
                        case '<5':
                            matchesDuracion = dur < 5;
                            break;
                        case '5-10':
                            matchesDuracion = dur >= 5 && dur < 10;
                            break;
                        case '10-15':
                            matchesDuracion = dur >= 10 && dur < 15;
                            break;
                        case '15-20':
                            matchesDuracion = dur >= 15 && dur <= 20;
                            break;
                        case '>20':
                            matchesDuracion = dur > 20;
                            break;
                    }
                }
    ...
    private toDraft(exercise: Exercise): ExerciseDraft {
        ...
        const dificultadTactica =
            typeof dif === 'object' && 'tactica' in dif ? (dif as any).tactica : 3;
        const dificultadTecnica =
            typeof dif === 'object' && 'tecnica' in dif ? (dif as any).tecnica : 3;
        const dificultadFisica =
            typeof dif === 'object' && 'fisica' in dif ? (dif as any).fisica : 3;
        const dificultadMental =
            typeof dif === 'object' && 'mental' in dif ? (dif as any).mental : 3;
    ...
    saveNewExercise(value: ExerciseFormValue) {
        ...
        this.exercisesApi.create(payload).subscribe({
            ...
            error: (err: any) => {
                ...
                const errorMsg =
                    err?.error?.message || err?.message || 'Error desconocido al crear ejercicio';
    ...
    private syncExerciseEquipment(
        ...
        const listCurrent = () =>
            new Promise<import('../services/exercise-equipment.api').ExerciseEquipmentRowDto[]>(
                (resolve, reject) => {
        ...
        return currentRowsPromise.then(async (currentRows) => {
            ...
            const createOne = (equipmentId: number) =>
                new Promise<void>((resolve, reject) => {
                    this.exerciseEquipmentApi
                        .createForExercise(exerciseId, { equipmentId })
                        .subscribe({
                            next: () => resolve(),
                            error: (e: unknown) => reject(e),
                        });
