...
export class Planning {
    ...
    // Roles que pueden eliminar planificaciones (Director Técnico y Entrenador)
    get canDelete(): boolean {
        const role = this.authService.getRoleSnapshot();
        return ['technical_director', 'coach'].includes(role || '');
    }
    ...
    private readonly plannings$ = this.refresh$.pipe(
        switchMap(() => {
            ...
            const from = this.filters.dateRange?.[0]
                ? this.toIsoDate(this.filters.dateRange[0])
                : undefined;
            const to = this.filters.dateRange?.[1]
                ? this.toIsoDate(this.filters.dateRange[1])
                : undefined;
                    ...
                    map((res: any) => {
                        ...
                        if (Array.isArray(res)) {
                            ...
                            const mapped = res.map((p: any) => {
                                ...
                                let assignedTo = '';
                                const targetType =
                                    p?.targetType === 'group' ? 'group' : 'individual';
                                ...
                                return {
                                    ...
                                    status: (['draft', 'active', 'archived'].includes(
                                        ...
                                        : 'draft') as any,
                                    version:
                                        p?.activeVersion?.versionNumber != null
                                            ? `v${p.activeVersion.versionNumber}`
                                            : p?.activeVersionId != null
                                              ? `v${p.activeVersionId}`
                                              : 'v1',
                                    author: authorName,
                                };
    ...
    readonly vm$ = combineLatest({
        ...
        map(({ items, loading, error }) => {
            ...
            const filtered = items.filter((p) => {
                ...
                if (search) {
                    const haystack =
                        `${p.name} ${p.assignedTo} ${p.targetType} ${p.author} ${p.version} ${p.status}`.toLowerCase();
        ...
        this.clubsApi.list().subscribe({
            next: (items) => {
                ...
                setTimeout(() => {
                    this.clubOptions = [
                        { label: 'Todos', value: 'Todos' },
                        ...this.clubs.map((c) => ({ label: c.name, value: String(c.id) })),
                    ];
                    this.cdr.markForCheck();
                });
            },
    ...
    exportCurrent(format: 'pdf' | 'csv'): void {
        ...
        this.toast.add({
            severity: 'info',
            summary: 'Exportación',
            detail: `Descargando ${format.toUpperCase()}…`,
        });
        ...
        if (!version) {
            ...
            this.toast.add({
                severity: 'warn',
                summary: 'Sin versión',
                detail: 'No hay versión disponible para exportar este planning.',
            });
            return;
        }

        const req$ =
            format === 'pdf' ? this.api.exportPdf(id, version) : this.api.exportCsv(id, version);
        ...
        req$.subscribe({
            ...
            error: (e: unknown) => {
                ...
                this.downloadingFormat = null;
                const msg =
                    e instanceof Error ? e.message : `No se pudo exportar ${format.toUpperCase()}.`;
    ...
    private async delete(item: PlanningListItem): Promise<void> {
        try {
            ...
            this.toast.add({
                severity: 'success',
                summary: 'Eliminada',
                detail: 'Planificación eliminada.',
            });
            this.refresh();
        } catch (e: any) {
            this.toast.add({
