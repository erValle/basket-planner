...
export class Clubs {
        ...
        switchMap(() => {
            ...
            return this.api.list().pipe(
                ...
                catchError((e: unknown) => {
                    this.zone.run(() => {
                        const msg =
                            e instanceof Error ? e.message : 'No se pudieron cargar los clubes.';
    ...
    vm$ = combineLatest([this.clubs$, this.loading$.pipe(startWith(true))]).pipe(
        map(([clubs, loading]) => {
            ...
            const filtered = clubs.filter((c) => {
                ...
                const matchesStatus =
                    this.filters.status === 'all' || c.status === this.filters.status;
    ...
    deleteClub(): void {
        ...
        if (this.draft.teams > 0) {
            this.toast.add({
                severity: 'warn',
                summary: 'No se puede eliminar',
                detail: `El club tiene ${this.draft.teams} equipo(s) asociado(s). Elimina los equipos primero.`,
            });
            return;
        }
        ...
        this.api.remove(this.draft.id).subscribe({
            next: () => {
                this.toast.add({ severity: 'success', summary: 'Ok', detail: 'Club eliminado.' });
                this.closeDialog();
                this.refresh();
            },
            error: (e: unknown) => {
                const msg = e instanceof Error ? e.message : 'No se pudo eliminar el club.';
                this.toast.add({ severity: 'error', summary: 'Error', detail: msg });
            },
        });
    }
