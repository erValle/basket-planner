...
export class Material {
    ...
    items$ = combineLatest([this.refresh$, this.clubContext.selectedClubId$]).pipe(
        switchMap(([, clubId]) => {
            ...
            return this.api
                ...
                .pipe(
                    ...
                    catchError((e: unknown) => {
                        const msg =
                            e instanceof Error ? e.message : 'No se pudo cargar el material.';
    ...
    vm$ = combineLatest([this.items$, this.loading$.pipe(startWith(true))]).pipe(
        map(([items, loading]) => {
            ...
            const filtered = items.filter((i) => {
                ...
                const matchesCat =
                    this.filters.category === 'all' || i.category === this.filters.category;
                const matchesStatus =
                    this.filters.status === 'all' || i.status === this.filters.status;
    ...
    private toUiItem(e: EquipmentDto): {
        id: number;
        name: string;
        category: string;
        status: string;
        ...
        return {
            id: Number(e.id),
            name: e.name,
            category: (e.characteristics as any)?.category
                ? String((e.characteristics as any).category)
                : 'Otros',
    ...
    statusTag(item: { id: number; name: string; category: string; status: string }) {
        if (item.status === 'available')
            return { label: 'Disponible', severity: 'success' as const };
        if (item.status === 'maintenance')
            return { label: 'Mantenimiento', severity: 'warn' as const };
