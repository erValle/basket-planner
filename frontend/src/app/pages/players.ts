...
export class Players {
    ...
    private readonly players$ = this.refresh$.pipe(
        switchMap(() => {
            ...
            return this.playersApi
                ...
                .pipe(
                    map((items: PlayerDto[] | null | undefined) =>
                        (items ?? []).map((p: PlayerDto) => this.toUiPlayer(p)),
                    ),
                    map((items) => {
        ...
        map(({ items, loading, error }) => {
            const filtered = items.filter((player) => {
                const matchesCategory =
                    this.filters.category === 'Todas' || player.category === this.filters.category;
                const matchesSearch =
                    this.filters.search === '' ||
                    player.name.toLowerCase().includes(this.filters.search.toLowerCase());
    ...
    private loadUnassignedUsers(): void {
        ...
        if (!selectedClubId) {
            ...
            this.selectionError = 'Debes seleccionar un club para ver usuarios disponibles.';
            this.availablePlayersForSelection = [];
            return;
        }

        // Cargar usuarios con rol 'user' que pertenecen al club seleccionado
        this.usersApi
            .list({ role: 'user', clubId: selectedClubId, page: 1, pageSize: 200 } as any)
            .subscribe({
                next: (res: any) => {
                    this.selectionLoading = false;
                    const items = Array.isArray(res?.items)
                        ...
                        : Array.isArray(res)
                          ? res
                          : [];
                    this.availablePlayersForSelection = items.map((u: any) =>
                        this.toSelectionItem(u),
                    );
                },
                error: (e: unknown) => {
                    this.selectionLoading = false;
    ...
    private loadFilters(): void {
        ...
        this.teamsApi
            .list({ clubId: selectedClubId != null ? String(selectedClubId) : undefined })
            .subscribe({
                next: (items) => {
                    this.teams = items ?? [];
                    this.teamOptions = [
    ...
    private toUiPlayer(p: PlayerDto): PlayerDetailModel {
        ...
        return {
            ...
            currentTeam: firstTeam
                ? `${firstTeam.name}${firstTeam.category ? ` - ${firstTeam.category}` : ''}`
                : '',
            weeklyLoad: '',
            lastFeedback: '',
        };
