...
export class PlanningEdit {
    ...
    equipmentOptions = computed(() =>
        this.equipment().map((e) => ({ label: e.name, value: String(e.name) })),
    );
    ...
    private translatePhase(phase: string | undefined | null): string {
        if (!phase) return '';
                ...
                blocks: [
                    {
                        ...
                        exercises: exercises.map((e: any) => ({
                            ...
                            notes: [
                                ...
                                e?.phase ? `Fase: ${this.translatePhase(e.phase)}` : null,
                                e?.difficulty
                                    ? `Dificultad: ${JSON.stringify(e.difficulty)}`
                                    : null,
    ...
    addExerciseFromDatabase(exercise: ExerciseDto): void {
        ...
        const diff = exercise.difficulty as
            | { tactica?: number; tecnica?: number; fisica?: number; mental?: number }
            | undefined;
    ...
    isExerciseValid(e: PlanningExerciseEditor): boolean {
        return e.name.trim().length > 0 && e.durationMin > 0;
    }
