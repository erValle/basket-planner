...
export class PlanningDetail {
        ...
        for (const s of sessions) {
                    ...
                    blocks.push({
                        ...
                        exercises: Array.isArray(b?.exercises)
                            ? b.exercises.map((e: any) => ({
                                  ...
                                  durationMin:
                                      e?.durationMin != null ? Number(e.durationMin) : undefined,
            ...
            blocks.push({
                ...
                exercises: exs.length
                    ? exs.map((e: any) => ({
                          id: String(
                              e?.exerciseId ?? e?.id ?? `e-${Math.random().toString(16).slice(2)}`,
                          ),
                          name: String(e?.name ?? 'Ejercicio'),
    ...
    private mapTrainingPlanToDetail(
        plan: any,
        versionRow: any,
        versionLabel: string,
    ): PlanningDetailResponse {
        ...
        const planIntensity = plan?.intensity
            ? intensityMap[plan.intensity] || plan.intensity
            : undefined;
        ...
        // Extraer usuarios asignados
        const assignedUsers = (Array.isArray(plan?.assignments) ? plan.assignments : [])
            .filter((a: any) => a?.user)
            .map((a: any) => ({
                id: a.user.id,
        ...
        return {
            ...
            date: this.formatIsoDate(
                versionRow?.date ?? plan?.activeVersion?.date ?? plan?.createdAt,
            ),
        ...
        this.api.get(this.planningId).subscribe({
            next: (plan) => {
                ...
                const activeVersionId =
                    plan?.activeVersionId != null ? String(plan.activeVersionId) : null;
                const resolvedVersionId =
                    selectedVersionId ??
                    activeVersionId ??
                    (versions[0]?.id != null ? String(versions[0].id) : null);
                ...
                this.api.getVersion(this.planningId!, resolvedVersionId).subscribe({
                    next: (versionRow) => {
                        ...
                        const resolved = this.mapTrainingPlanToDetail(
                            plan,
                            versionRow,
                            versionLabel,
                        );
                        ...
                        this.versionOptions = (resolved.versions ?? []).map((v: any) => ({
                            label: v.label,
                            value: v.value,
                        }));
    ...
    canAcceptPlanning(): boolean {
        // Solo Director Técnico y Entrenador pueden activar planificaciones
        const role = this.userContext.getRoleSnapshot();
        const canManage = role === 'technical_director' || role === 'coach';
        return canManage && this.detail?.status === 'draft';
    }

    canManagePlanning(): boolean {
        // Solo Director Técnico y Entrenador pueden crear versiones, editar, etc.
        const role = this.userContext.getRoleSnapshot();
    ...
    private readonly phaseTranslations: Record<string, string> = {
    ...
    translatePhase(phase: string | undefined | null): string {
        if (!phase) return '';
        const lowerPhase = phase.toLowerCase();
        return this.phaseTranslations[lowerPhase] || phase;
    }
    ...
    exportPdf(): void {
        ...
        if (!this.selectedVersion) {
            this.toast.add({
                severity: 'warn',
                summary: 'Selecciona una versión',
                detail: 'No hay versión seleccionada para exportar.',
            });
            return;
        }
        ...
        this.api.exportPdf(this.planningId, this.selectedVersion).subscribe({
            next: (blob) => {
                ...
                this.downloadBlob(
                    blob,
                    `planning-${this.planningId}-${this.selectedVersion ?? 'latest'}.pdf`,
                );
    ...
    exportCsv(): void {
        ...
        this.api.exportCsv(this.planningId, this.selectedVersion).subscribe({
            next: (blob) => {
                ...
                this.downloadBlob(
                    blob,
                    `planning-${this.planningId}-${this.selectedVersion ?? 'latest'}.csv`,
                );
                // Cerrar el toast de descarga
                this.toast.clear('download-toast');
                this.toast.add({
    ...
    private downloadBlob(blob: Blob, filename: string): void {
        ...
        this.toast.add({
            severity: 'success',
            summary: 'Exportación',
            detail: `Descarga iniciada: ${filename}`,
        });
    }
    ...
    getDifficultyValue(key: string): number {
        if (
            !this.selectedExercise?.difficulty ||
            typeof this.selectedExercise.difficulty !== 'object'
        ) {
            return 0;
        }
