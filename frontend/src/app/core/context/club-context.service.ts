        ...
        this.playerClubsApi.listMemberships(String(userId)).subscribe({
            next: (res) => {
                ...
                const clubIds = Array.isArray(memberships)
                    ? memberships.map((m: any) => m?.clubId).filter((id: any) => id != null)
                    : [];
                const uniqueClubIds = Array.from(
                    new Set(clubIds.map((x: any) => Number(x)).filter((x) => Number.isFinite(x))),
                );
                ...
                this.clubsApi.list().subscribe({
                    next: (allClubs) => {
                        const allowed = (allClubs ?? []).filter((c) =>
                            uniqueClubIds.includes(Number(c.id)),
                        );
