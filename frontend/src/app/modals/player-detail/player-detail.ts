...
export class PlayerDetail {
    ...
    private loadClubs(): void {
        // Cargar todos los clubes para permitir transferencias a cualquier club
        this.clubsApi.list({ includeAll: true }).subscribe({
            next: (clubs) => {
                ...
                this.api.listMemberships(this.player.id).subscribe({
                    ...
                    error: (e2: unknown) => {
                        ...
                        const msg =
                            e2 instanceof Error
                                ? e2.message
                                : 'No se pudieron cargar las pertenencias.';
    ...
    confirmAdd(): void {
        ...
        if (!this.addForm.clubId || !this.addForm.startDate) {
            this.toast.add({
                ...
                detail: 'Club y fecha son obligatorios.',
            });
            return;
        }
    ...
    confirmClose(): void {
        ...
        if (!this.closeForm.endDate) {
            this.toast.add({
                severity: 'warn',
                summary: 'Revisa el formulario',
                detail: 'Fecha fin es obligatoria.',
            });
            return;
        }
        this.savingMembership = true;
        this.api
            .closeMembership(this.selectedMembership.id, {
                endDate: this.toIsoDate(this.closeForm.endDate),
            })
            .subscribe({
                next: () => {
                    this.savingMembership = false;
                ...
                error: (e: unknown) => {
                    this.savingMembership = false;
                    const msg =
                        e instanceof Error ? e.message : 'No se pudo cerrar la pertenencia.';
                    this.toast.add({ severity: 'error', summary: 'Error', detail: msg });
                },
            });
    }
    ...
    markPrimary(m: PlayerMembership): void {
        ...
        if (m.status !== 'active') {
            this.toast.add({
                ...
                detail: 'Solo se puede marcar como principal una pertenencia activa.',
            });
            return;
        }
    ...
    nextTransfer(): void {
        if (this.transferStep === 0 && !this.canProceedTransfer()) {
            this.toast.add({
                severity: 'warn',
                summary: 'Revisa el formulario',
                detail: 'Selecciona club y fecha.',
            });
            return;
        }
            ...
            .subscribe({
                next: () => {
                    ...
                    this.toast.add({
                        severity: 'success',
                        summary: 'Transferido',
                        detail: 'Transferencia registrada.',
                    });
                    this.loadMemberships();
                },
                error: (e: unknown) => {
