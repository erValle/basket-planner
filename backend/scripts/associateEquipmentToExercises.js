...
function findMatchingEquipment(material, allEquipment) {
  ...
  for (const [equipName, keywords] of Object.entries(equipmentMapping)) {
    const matches = keywords.some(
      (kw) => materialLower.includes(kw.toLowerCase()) || kw.toLowerCase().includes(materialLower)
    );
    ...
    if (matches) {
      ...
      const equipment = allEquipment.find((e) => {
        ...
        return keywords.some(
          (kw) => eName.includes(kw.toLowerCase()) || kw.toLowerCase().includes(eName)
        );
      });
...
async function associateEquipment() {
  try {
    ...
    if (allEquipment.length === 0) {
      console.log(' No hay equipment en la base de datos. Ejecuta el seeder primero.');
      return;
    }
    ...
    for (const exercise of exercises) {
      ...
      if (exercise.equipmentItems && exercise.equipmentItems.length > 0) {
        skippedCount++;
        console.log(
          `➖ ${exercise.name} - Ya tiene ${exercise.equipmentItems.length} materiales asociados`
        );
        continue;
      }
      ...
      if (!Array.isArray(materials) || materials.length === 0) {
        console.log(` ${exercise.name} - No tiene materiales definidos`);
        continue;
      }
      ...
      if (matchedEquipment.length > 0) {
        ...
        console.log(
          `   Materiales: ${matchedEquipment.map((m) => `${m.equipment.name} (x${m.quantity})`).join(', ')}`
        );
      } else {
        console.log(` ${exercise.name} - No se encontraron matches para: ${materials.join(', ')}`);
      }
    }
    ...
    if (stats.unmatchedMaterials.size > 0) {
      console.log('\n Materiales sin match (considera añadirlos a equipment):');
