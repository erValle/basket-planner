...
async function testRecommender() {
  try {
    ...
    exercises.slice(0, 5).forEach(ex => {
      ...
      console.log(`      Dificultad: T:${difficulty.tactica || '?'} Té:${difficulty.tecnica || '?'} F:${difficulty.fisica || '?'} M:${difficulty.mental || '?'}`);
    ...
    plan.sessions.forEach((session, sessionIdx) => {
      ...
      printInfo('  MaxDuration respetado', session.metrics.maxDurationRespected ? '✅' : '❌');
    ...
    const repetitionRate = ((totalExercises - uniqueExercises.size) / totalExercises * 100).toFixed(1);
    ...
    console.log('\n' + colors.bright + colors.green + '  🎉 El motor de recomendación está funcionando correctamente!' + colors.reset + '\n');
