...
async function validateFeedbackSystem() {
  ...
  try {
    ...
    if (indexes.length === 0) {
      console.log('    Índice feedbacks_version_session_idx no encontrado (no crítico)');
    } else {
    ...
    const [versions] = await sequelize.query(
      'SELECT COUNT(*) as count FROM training_plan_versions'
    );
    ...
    if (users[0].count === 0 || plans[0].count === 0 || versions[0].count === 0) {
      console.log('    Faltan datos de ejemplo');
    ...
    if (existingFeedbacks.length > 0) {
      ...
      console.log('   No hay feedbacks todavía (esperado en sistema nuevo)\n');
    }
