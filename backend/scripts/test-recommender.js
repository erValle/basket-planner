...
async function testRecommender() {
  ...
  try {
    ...
    console.log(
      `   Sesiones generadas: ${result1.sessions.length}/${test1Params.numberOfSessions}`
    );
    ...
    result1.sessions.forEach((session, idx) => {
      ...
      console.log(
        `     - Fases: ${[...new Set(session.exercises.map((e) => e.phase))].join(', ')}`
      );
    });
    ...
    console.log(
      `   Sesiones generadas: ${result2.sessions.length}/${test2Params.numberOfSessions}`
    );
    ...
    result2.sessions.forEach((session, idx) => {
      ...
      console.log(
        `     - Por fase: ${Object.entries(phases)
          .map(([k, v]) => `${k}(${v})`)
          .join(', ')}`
      );
    });
    ...
    console.log(`\n${result3.wasAborted ? '' : ''} Completado en ${time3}ms`);
    console.log(
      `   Sesiones generadas: ${result3.sessions.length}/${test3Params.numberOfSessions}`
    );
    ...
    console.log('═══════════════════════════════════════════════════════════');
    console.log(
      `Test 1 (simple): ${result1.sessions.length}/${test1Params.numberOfSessions} sesiones en ${time1}ms`
    );
    console.log(
      `Test 2 (etiquetas): ${result2.sessions.length}/${test2Params.numberOfSessions} sesiones en ${time2}ms`
    );
    console.log(
      `Test 3 (abort): ${result3.sessions.length}/${test3Params.numberOfSessions} sesiones en ${time3}ms (abortado: ${result3.wasAborted})`
    );

    const avgTimePerSession =
      (time1 / test1Params.numberOfSessions + time2 / test2Params.numberOfSessions) / 2;
