    ...
    try {
      ...
      if (result1.sessions && result1.sessions.length > 0) {
        result1.sessions.forEach((s, i) => {
          console.log(`   Sesión ${i+1}: ${s.exercises?.length || 0} ejercicios, ${s.metrics?.durationMinutes || 0} min`);
        });
      }
    } catch (error) {
      console.error(`\n❌ Error: ${error.message}`);
    ...
    try {
      ...
      if (result4.sessions && result4.sessions.length > 0) {
        result4.sessions.forEach((s, i) => {
          const phases = {};
          (s.exercises || []).forEach(e => { phases[e.phase] = (phases[e.phase] || 0) + 1; });
          console.log(`   Sesión ${i+1}: ${s.exercises?.length || 0} ejercicios | ${Object.entries(phases).map(([k,v]) => `${k}:${v}`).join(', ')}`);
        });
      }
    } catch (error) {
      console.error(`\n❌ Error: ${error.message}`);
