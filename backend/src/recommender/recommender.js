...
function normalizeGoals(inputGoals) {
  ...
  for (const goal of inputGoals) {
    ...
    for (const [frontendKey, modelGoals] of Object.entries(config.frontendObjectiveToGoals || {})) {
      if (frontendKey.toLowerCase().includes(lowerGoal) || lowerGoal.includes(frontendKey.toLowerCase())) {
 ...
 */
async function selectExercisesForPhase(availableExercises, context, targetDuration, phase, maxDuration = null) {
  ...
  if (scorerInstance && modelInstance?.isTrained) {
    scoredExercises = await scorerInstance.scoreExercises(scoringContext, phaseExercises, sessionState);
  } else {
    // Fallback a heurístico
    const heuristicScorer = new HeuristicScorer();
    scoredExercises = phaseExercises.map(exercise => {
  ...
  for (const phase of phases) {
    ...
    for (const exercise of phaseExercises) {
      ...
      sessionExercises.push({
        ...
        material: exercise.materiales_necesarios || (exercise.tags && exercise.tags.materiales) || [],
        // Campos de scoring
...
async function generatePlan(allExercises, planParams, options = {}) {
  ...
  const HIGH_FIT_THRESHOLD = exerciseScarcity < 0.5 ? 0.60 : 0.80;  // Más permisivo con pocos ejercicios
  const MEDIUM_FIT_THRESHOLD = exerciseScarcity < 0.5 ? 0.40 : 0.60; // Más permisivo
  ...
  console.log(`   Ejercicios disponibles: ${filteredExercises.length}, escasez: ${exerciseScarcity.toFixed(2)}`);
  ...
  for (const phase of phases) {
    ...
    let scoredExercises;
    if (scorerInstance && modelInstance?.isTrained) {
      console.log(`   Scoring phase ${phase.name} con modelo TFRS (${phaseExercises.length} ejercicios)...`);
      scoredExercises = await scorerInstance.scoreExercises(scoringContext, phaseExercises, {});
    } else {
      console.log(`   Scoring phase ${phase.name} con heurístico (${phaseExercises.length} ejercicios)...`);
      const heuristicScorer = new HeuristicScorer();
      scoredExercises = phaseExercises.map(exercise => {
        const score = heuristicScorer.calculateScore(scoringContext, exercise);
        return { exercise, score, source: 'heuristic_fallback' };
  ...
  for (let sessionIdx = 0; sessionIdx < numberOfSessions; sessionIdx++) {
    ...
    for (const phase of phases) {
      ...
      const candidatesWithPriority = phasePool.map(item => {
      ...
      for (const candidate of candidatesWithPriority) {
        ...
        sessionExercises.push({
          ...
          fitLevel: candidate.score >= HIGH_FIT_THRESHOLD ? 'high' : 
                    candidate.score >= MEDIUM_FIT_THRESHOLD ? 'medium' : 'low',
          isRepeated,
  ...
  const repeatedExercises = Array.from(exerciseUsageCount.values()).filter(count => count > 1).length;
  const highFitRepeats = sessions.reduce((sum, s) => 
    sum + s.exercises.filter(e => e.isRepeated && e.fitLevel === 'high').length, 0);
  ...
  return {
    ...
    summary: {
      ...
      uniqueExercises,
      repeatedExercises,
      exerciseVariety: totalExercises > 0 ? (uniqueExercises / totalExercises * 100).toFixed(1) + '%' : '0%',
