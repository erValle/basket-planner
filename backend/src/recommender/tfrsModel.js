...
class TFRSRankingModel {
  ...
  _buildExerciseTower(inputDim, exerciseVocabSize) {
	...
	const idEmbedding = tf.layers.embedding({
	...
	const combined = tf.layers.concatenate({ name: 'combine_features_embedding' }).apply([x, flatIdEmbedding]);
  ...
  buildModel(exerciseData = []) {
	...
	const exerciseFeaturesInput = tf.input({ shape: [dims.exerciseInputDim], name: 'exercise_features' });
	...
	for (let i = 0; i < architecture.rankingLayers.length; i++) {
	  ...
	  rankingLayer = tf.layers.dropout({
		rate: architecture.dropoutRate,
  ...
  encodeContextFeatures(context) {
	...
	features.push(...this.vocabularyEncoders.intensity.encode(context.intensity || defaults.intensity));
	features.push(...this.vocabularyEncoders.sessionPhase.encode(context.sessionPhase || defaults.sessionPhase));
	features.push(...this.vocabularyEncoders.position.encode(context.position || defaults.position));
	
	features.push(this._normalizeMinutes(context.sessionDurationMinutes || defaults.sessionDurationMinutes, 0, 180));
	features.push(this._normalizeMinutes(context.currentSessionMinutes || 0, 0, 180));
	features.push(this._normalizeMinutes(context.remainingMinutes || defaults.sessionDurationMinutes, 0, 180));
	...
	features.push(this._normalizeCount(context.targetExerciseCount || defaults.targetExerciseCount, 1, 20));
