...
class TFRSTrainer {
  ...
  _generateRandomContext() {
    ...
    return {
      intensity: vocabularies.intensities[Math.floor(Math.random() * vocabularies.intensities.length)],
      sessionPhase: vocabularies.sessionPhases[Math.floor(Math.random() * vocabularies.sessionPhases.length)],
...
class HeuristicScorer {
  ...
  _calculateDifficultyFitScore(context, exercise) {
    ...
    const difficultyRange = config.intensityToDifficulty[intensity.toLowerCase()] || 
  ...
  _calculateVarietyScore(context, exercise) {
    ...
    switch (typeCount) {
      case 0: return 1.0;   // 1er uso
      case 1: return 0.75;  // 2do uso
      case 2: return 0.50;  // 3er uso
      case 3: return 0.25;  // 4to uso
      default: return 0.0;  // 5to+
    }
  }
  ...
  _calculateUniquenessScore(context, exercise) {
    ...
    switch (usageCount) {
      case 0: return 1.0;   // Primera aparición
      case 1: return 0.5;   // Segunda aparición
      default: return 0.0;  // Tercera+
    }
  }
}
